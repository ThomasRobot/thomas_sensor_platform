...
dataDir = '/mnt/DataBlock2/thomas/YQ-south/calibration';

Nimages = length(dir([dataDir '/image_00/*.png']));
gridsize = 0.1;
baseline = 0.5;
marginthres = 0.2;
depththres = 0.6;
...
calib = readCalibrationFromOpenCV(...
  '/mnt/DataBlock2/thomas/YQ-south/calibration/left.yaml', ...
  '/mnt/DataBlock2/thomas/YQ-south/calibration/right.yaml');
IntrinsicMatrix = calib.K_{1};

if ~undistorted
  istorted
  cameraParams = cameraParameters('IntrinsicMatrix',IntrinsicMatrix.','RadialDistortion', calib.D_{1},'WorldUnits','m');
else
...
fn_cloud = cell(0);
for i = 0:Nimages-1
  fn_image{end+1} = [dataDir '/image_00/left_' num2str(i, '%02d') '.png'];
  fn_cloud{end+1} = [dataDir '/velodyne/cloud_' num2str(i, '%02d') '.pcd'];
  
  %     fn_cloud{end+1} = [dataDir '/cloud.' num2str(i) '.ply'];
  if ~undistorted
    I = imread([dataDir '/image_00/left.' num2str(i) '.png']);
    I1 = undistortImage(I, cameraParams);
    % imwrite(I1,[dataDir '/image_00/left.' num2str(i) '.png'],'png')
  end
end
